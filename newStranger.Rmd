...
Questions for Gina:
* What graphs could/should I make?
* What is best approach?
* How much is reasonable to expect kids of varying skills to do on their own in a short time?  Shiny app?  
* Shiny apps + ggplot2
...
char_scenes <- scenes  %>%
      select(character, seasonNum, episode_id, scene_id, scene_length) %>%
      distinct() %>%
      arrange(character, scene_id) %>%
      group_by(character) %>%
      summarize(num_scenes = n_distinct(scene_id),
